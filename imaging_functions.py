...
def show_map_3d(gen_obj,img_x,img_y,sca,centre_offset=[0,0],num_samples = 1,channel=-1, octaves=1,neg_octaves=0, fade=0.5):
    ...
    if num_samples == 1:
        if extra_args:
            Z = np.asarray([[gen_obj.get_height(a,b,channel=channel, octaves=octaves,neg_octaves=neg_octaves, fade=fade) for a in x] for b in y])
        else:
            Z = np.asarray([[gen_obj.get_height(a,b) for a in x] for b in y])
    else:
        ...
        for _ in range(num_samples):
            ...
            offset_y += Y
            if extra_args:
                Z += np.asarray([[gen_obj.get_height(a,b,channel=channel, octaves=octaves,neg_octaves=neg_octaves, fade=fade) for a in x] for b in y])/num_samples
            else:
                Z += np.asarray([[gen_obj.get_height(a,b) for a in x] for b in y])/num_samples
